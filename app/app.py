...
def _write_preset_yaml(
    ...
    # CLAUDE-NOTE: Klippbok's root config validator requires either
    # `dataset.path` or at least one entry in `datasets`. Without this
    # block the yaml loads but fails validation, which bubbles up as
    #   "No dataset paths provided. Add at least one entry to 'datasets'"
    # at every Run. Only seed when the user hasn't already set it so a
    # multi-source setup they authored by hand survives preset changes.
    # "." resolves from the yaml's location (= work_dir per the schema
    # description), so the yaml stays portable if the folder is moved.
    existing_datasets = existing.get("datasets")
    if not (isinstance(existing_datasets, list) and existing_datasets):
        existing["datasets"] = [{"path": "."}]

    header = (
        f"# Auto-generated by klippbok-pinokio launcher.\n"
        f"# Preset: {preset_name}\n"
        f"# The launcher merges `video.*` on every Target Trainer preset\n"
        f"# change and seeds `datasets: [path: .]` once so Klippbok's\n"
        f"# root validator passes; other sections (dataset, controls,\n"
        f"# quality, …) are preserved if you've edited them.\n\n"
    )
    try:
